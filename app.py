...
def init_db():
    ...
    with get_db() as db:
        ...
        # ── Детали ──
        parts = [
            "Аккумулятор", "Процессор", "Оперативная память",
            "Модем связи", "Стекло дисплейного модуля", "Дисплейная матрица"
        ]
        for p in parts:
        ...
        # ── Диагнозы ──
        diagnoses = [
            "Исправен", "Разряжен аккумулятор", "Неисправен аккумулятор",
            "Перегрев процессора", "Слабый сигнал модема",
            "Трещины на стекле", "Артефакты на матрице дисплея",
        ]
        for d in diagnoses:
        ...
        for diag_name, char_rules in rules.items():
            ...
            for char_name, min_v, max_v, fixed_v in char_rules:
                ...
                db.execute(
                    ...
                    "(diagnosis_id,characteristic_id,min_val,max_val,fixed_val) VALUES (?,?,?,?,?)",
        ...
        repair_data = {
            ...
            "Ремонт перегрева процессора": [
                ...
                (4, "Если не помогло: выключить, вскрыть, заменить термоинтерфейсы, протестировать", None),
            ],
...
# Нормальные значения признаков (середина исправного диапазона)
# Используются при генерации датасета для признаков «не участвующих в диагнозе»
# и при инференсе ML для заполнения не введённых пользователем полей
FEATURE_NORMAL = {
...
# Середина нормального диапазона для инференса
FEATURE_NORMAL_DEFAULT = {k: (v[0] + v[1]) / 2 for k, v in FEATURE_NORMAL.items()}
...
def generate_dataset(n_per_class=700):
    """
    Генерирует синтетический датасет на основе правил БД.
    Формула: x_i ~ U(a_{i,c}, b_{i,c}) — Глава 4.2.1

    Ключевые улучшения по сравнению с базовой реализацией:
    - Трещины на стекле: площадь строго > 0 (минимум 1 мм²)
    - Неисправен аккумулятор: половина объектов со вздутием=1, половина=0
    - Признаки вне диагноза генерируются из нормального диапазона
    - n_per_class=700 для лучшего обучения
    """
    ...
    for diag_name, char_rules in rules.items():
        rule_map = {r["name"]: r for r in char_rules}

        for sample_idx in range(n_per_class):
            row = []
            for feat in FEATURE_ORDER:
                norm_lo, norm_hi = FEATURE_NORMAL.get(feat, (0, 100))

                if feat in rule_map:
                    r = rule_map[feat]

                    # --- специальная логика для "Наличие вздутия аккумулятора" ---
                    if feat == "Наличие вздутия аккумулятора":
                        if r["fixed_val"] is not None:
                            val = float(r["fixed_val"])
                        else:
                            # "Неисправен аккумулятор": чередуем 0 и 1
                            # чётные объекты — вздутие=0, нечётные — вздутие=1
                            val = float(sample_idx % 2)
                        row.append(val)
                        continue
                    ...
                    if r["fixed_val"] is not None:
                        ...
                        lo = r["min_val"] if r["min_val"] is not None else norm_lo
                        hi = r["max_val"] if r["max_val"] is not None else norm_hi

                        # --- специальная логика для "Трещины на стекле" ---
                        if diag_name == "Трещины на стекле" and feat == "Площадь трещин на стекле":
                            lo = max(lo, 1)   # площадь трещин строго > 0

                        # Защита от lo == hi (чтобы uniform не упал)
                        if lo >= hi:
                            val = lo
                        else:
                    ...
                    if norm_lo >= norm_hi:
                        val = norm_lo  # бинарный: norm=(0,0) → val=0
                    else:
                        val = round(rng.uniform(norm_lo, norm_hi), 3)
...
def train_model():
    ...
    """
    X, y = generate_dataset(n_per_class=700)
...
def ml_predict(values_dict):
    ...
    Пропущенные признаки заполняются серединой нормального диапазона.
    """
    ...
    for feat in FEATURE_ORDER:
        if feat in values_dict and values_dict[feat] is not None and values_dict[feat] != "":
            ...
            row.append(float(FEATURE_NORMAL_DEFAULT.get(feat, 0)))
    X     = np.array([row])
    proba = clf.predict_proba(X)[0]
    result = sorted(zip(enc.classes_, proba), key=lambda x: -x[1])
...
def hybrid_diagnose(values_dict):
    ...
    if len(expert_candidates) == 1:
        ...
        final  = ml_filtered[0][0] if ml_filtered else expert_candidates[0][0]
        method = "hybrid"
    else:
...
if __name__ == "__main__":
    ...
    clf, _ = load_ml_model()
    if clf is None:
        print("⚙️  Первый запуск: обучение ML-модели (~30–60 сек)...")
        metrics = train_model()
